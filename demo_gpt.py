...
def save_trajectory_to_csv(trajs_e, filename):
    with open(filename, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['x', 'y'])  # Write header
        for point in trajs_e[0]:  # Assuming trajs_e has shape (1, S, 2)
            writer.writerow([point[0].item(), point[1].item()])
...
def main(
    ...
    for si in idx:
        ...
        csv_filename = f'trajectory_{model_name}_{si}.csv'
        save_trajectory_to_csv(trajs_e, csv_filename)
