...
def run_model(model, rgbs, points, S_max=128, iters=16, sw=None):
    ...
    # zero-vel init
    trajs_e = points.unsqueeze(1).repeat(1, S, 1)
...
def main(
        ...
        points_file='./image0.csv',  # Update this path
        S=48, # seqlen
    ...
    points = read_points_from_csv(points_file)
    ...
    for si in idx:
        ...
        with torch.no_grad():
            trajs_e = run_model(model, rgb_seq, points, S_max=S, iters=iters, sw=sw_t)
        ...
        save_trajectory_to_csv(trajs_e, f'{model_name}_trajectory.csv')
            
    writer_t.close()

# Add a function to save the trajectory to a CSV file
def save_trajectory_to_csv(trajs, filename):
    trajs = trajs.cpu().numpy()[0]  # Convert to numpy array and remove the batch dimension
